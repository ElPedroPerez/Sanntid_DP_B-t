...
void convertToSeriellPrint(float input)
{
  if (input < 10.00 && input >= 0.00)
  {
    Serial.print("???");
  }
  if (input < 100.00 && input >= 10.00)
  {
    Serial.print("??");
  }
  if (input <= 180.00 && input >= 100.00)
  {
    Serial.print("?");
  }
  
  if (input < 0.00 && input >= -9.99)
  {
    Serial.print("??");
  }
  if (input <= -10.00 && input > -100.00)
  {
    Serial.print("?");
  }
...
void output_angles()
{
  ...
  else if (output_format == OUTPUT__FORMAT_TEXT)
  {
    ...
    float yaw_degree = TO_DEG(yaw);
    convertToSeriellPrint(yaw_degree);
    Serial.print(yaw_degree); Serial.print(":");

    float pitch_degree = TO_DEG(pitch);
    convertToSeriellPrint(pitch_degree);
    Serial.print(pitch_degree); Serial.print(":");

    float roll_degree = TO_DEG(roll);
    convertToSeriellPrint(roll_degree);
    Serial.print(roll_degree); Serial.print(">");
  }
